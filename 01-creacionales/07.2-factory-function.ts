...
function createLogger(level: LogLevel) {
  ...
  return function (message: string) {
    ...
    switch (level) {
      case "info":
        alert = `[INFO ${formatDate(new Date())}] ${message}`;
        console.log(`%c${alert}`, COLORS.blue);

        break;

      case "error":
        alert = `[ERROR ${formatDate(new Date())}] ${message}`;
        console.log(`%c${alert}`, COLORS.red);
        break;

      case "warn":
        alert = `[WARNING  ${formatDate(new Date())}] ${message}`;
        console.log(`%c${alert}`, COLORS.yellow);
        break;
