 ...
 * * se sabe de antemano qué tipo de procesamiento se    o en qué orden
...
class Supervisor extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 1000) {
      console.log("%cMonto aprobado por SUPERVISOR", COLORS.blue);
      return;
    }
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 5000) {
      console.log("%cMonto aprobado por MANAGER", COLORS.yellow);
      return;
    }
...
class Director extends BaseApprover {
  override approveRequest(amount: number): void {
    //  if (amount >5000) {
    console.log("%cMonto aprobado por DIRECTOR", COLORS.green);
    // return;
    // }
